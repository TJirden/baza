...
public class MemeVectorRepository implements IdRepository {
  ...
  @Override
  public List<String> findSimilarIds(
      String query, int limit, Long userId, List<Long> userGroupIds) {
    ...
    return vectorStore.similaritySearch(request).stream().map(Document::getId).toList();
  }

  @Override
  public List<String> findSimilarFileIds(
      String query, int limit, Long userId, List<Long> userGroupIds) {
    SearchRequest request =
        SearchRequest.builder()
            .query(query)
          ...
          jdbcTemplate.queryForObject(
              ...
              (rs, rowNum) -> {
                ...
                try {
                  ...
                  Long ownerId =
                      metadata.containsKey("ownerId")
                          ? Long.parseLong(String.valueOf(metadata.get("ownerId")))
                          : null;
