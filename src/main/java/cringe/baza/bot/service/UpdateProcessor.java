...
public class UpdateProcessor {
  ...
  private SendMessage processImageSave(Update update) {
    ...
    if (update.message().photo() == null || update.message().photo().length == 0) {
      ...
      return new SendMessage(
          chatId, "Ошибка: я не вижу фото в твоем сообщении. Сбрасываю состояние");
    }
    String description = update.message().caption();

    if (description == null || description.isBlank()) {
      return new SendMessage(
          chatId, "Пожалуйста, добавь описание к фото (в подписи), чтобы я мог его найти!");
    }
  ...
  private AnswerInlineQuery handleInlineQuery(InlineQuery inlineQuery) {
    ...
    try {
      ...
      InlineQueryResultCachedPhoto[] results =
          fileIds.stream()
              .map(
                  fileId -> {
