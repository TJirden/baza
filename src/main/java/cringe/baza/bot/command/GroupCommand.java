...
public class GroupCommand implements Command {
    ...
    public BaseRequest<?, ?> handle(Update update) {
        ...
        if (text == null || text.isBlank()) {
            return new SendMessage(chatId, "Использование:\n/group create {имя}\n/group join {id}\n/group leave {id}\n/group list");
        }

        String[] parts = text.split("\\s+", 2);
        String action = parts[0].toLowerCase();
        
        TelegramUser user = userService.getOrCreateUser(userId, update.message().from().username(), update.message().from().firstName());
