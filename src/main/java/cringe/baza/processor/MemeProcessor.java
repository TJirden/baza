...
public class MemeProcessor {
  ...
  public List<Meme> getMemesByDescription(
      String description, int limit, Long userId, List<Long> userGroupIds) {
    List<String> ids = idRepository.findSimilarIds(description, limit, userId, userGroupIds);

    return ids.stream().map(idRepository::findById).flatMap(Optional::stream).toList();
  }

  /** Поиск списка Telegram File ID по описанию для Inline Mode */
  public List<String> getFileIdsByDescription(
      String description, int limit, Long userId, List<Long> userGroupIds) {
  ...
  public Optional<Meme> getSingleMemeByDescription(
      String description, Long userId, List<Long> userGroupIds) {
  ...
  public boolean update(String id, String newDescription) {
    ...
    idRepository.save(
        id,
        new Meme(
            id, newDescription, meme.fileId(), meme.ownerId(), meme.visibility(), meme.groupIds()));
    return true;
  }
}
